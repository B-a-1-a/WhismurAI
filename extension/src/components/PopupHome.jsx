...
export function PopupHome({ isTranslating, setIsTranslating, setShowFloatingOverlay, targetLang, setTargetLang }) {
  const [status, setStatus] = useState("idle");
  const [targetLanguage, setTargetLanguage] = useState(() => {
    // Map targetLang code to language value
    const match = LANGUAGE_OPTIONS.find((lang) => lang.code === targetLang);
    return match ? match.value : "spanish";
  });
  ...
  // Check for voice models on mount
  useEffect(() => {
    ...
    // Listen for voice cloning updates
    const handleMessage = (msg) => {
      if (msg.type === 'VOICE_CLONE_STATUS') {
  ...
  // Update parent targetLang when language changes
  useEffect(() => {
    if (targetLangCode && setTargetLang) {
      setTargetLang(targetLangCode);
    }
  }, [targetLangCode, setTargetLang]);
  ...
  const handleToggleClonedVoice = () => {
    const newValue = !useVoiceClone;
    setUseVoiceClone(newValue);
    
    if (currentVoiceModel?.model_id) {
      chrome.runtime.sendMessage({
        type: 'SET_USE_CLONED_VOICE',
        useClonedVoice: newValue,
        modelId: currentVoiceModel.model_id,
        fishApiKey: '' // Will be retrieved from storage if needed
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[#9CA3AF]" style={{ fontSize: "12px" }}>
              {hasVoiceModel 
                ...
                : 'AI Voice Cloning (Fish AI) - Start translation to clone'}
            </span>
          </div>
          <Switch
            id="voice-clone"
            ...
            disabled={!hasVoiceModel || !isTranslating}
