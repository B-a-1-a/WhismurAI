...
export function PopupHome({ isTranslating, setIsTranslating, transcripts, setTranscripts }) {
  ...
  const handleStopTranslation = async () => {
    ...
    if (transcripts && transcripts.length > 0) {
      try {
        const transcriptTexts = transcripts.map(t => t.text || t).filter(t => t && t.trim());
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      {!isTranslating && transcripts.length > 0 && (
        <Button
          variant="outline"
          size="sm"
