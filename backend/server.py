...
from bot import (
    run_translation_bot,
)  # Full translation pipeline (STT → Translation → TTS)
...
async def clone_voice(
    audio: UploadFile = File(...), url: str = Form(...), title: str = Form(None)
):
    """
    ...
    try:
        ...
        print(
            f"[Voice Clone] Audio file: {audio.filename}, content_type: {audio.content_type}"
        )
        ...
        result = await voice_manager.create_voice_model(
            audio_data=audio_data, url=url, title=title
        )
...
async def get_voice_model(url: str):
    ...
    try:
        ...
        if model is None:
            raise HTTPException(
                status_code=404, detail="Voice model not found for this URL"
            )

        return JSONResponse(content={"status": "success", "data": model})
...
async def delete_voice_model(url: str):
    ...
    try:
        ...
        return JSONResponse(
            content={"status": "success", "message": "Voice model deleted"}
        )
